public class WeaknessEffect : ModifierEffect
{
    public WeaknessEffect(EffectAmount amount, int? duration = null, Enums.ModifierType level = Enums.ModifierType.Base, Enums.TargetType targetType = Enums.TargetType.Self, Enums.Stat statType = Enums.Stat.None) : base(amount, duration, level, targetType, statType)
    {
    ...
    public override ModifierEffect Copy(ModifierEffect effect)
    {
        ...
        return new WeaknessEffect(amount, Duration, Level, TargetType, StatType);
    }

    public override Enums.BuffType GetBuffType()
    {
        return Enums.BuffType.Weakness;
    }
    ...
    {
        return base.GetDisplayName() + " <sprite=0>";
    }
